...
float Kp = 50.0;   // Proportional: Main balancing force
float Ki = 300.0;    // Integral: Corrects drift over time
float Kd = 2.1;    // Derivative: Dampens oscillations
...
void handleUDP() {
    ...
    if (packetSize) {
        ...
        // Parse commands: "Kp:15.0,Ki:0.5,Kd:0.8,SP:10.0"
        ...
        snprintf(reply, sizeof(reply), 
                "Angle:%.2f,SP:%.2f,Kp:%.2f,Ki:%.2f,Kd:%.2f,Active:%d", 
                getPitchAngle(), setpoint, Kp, Ki, Kd, robot_active);
